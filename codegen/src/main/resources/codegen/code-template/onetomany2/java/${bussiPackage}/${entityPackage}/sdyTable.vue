  ...
  <div class="pubOut3rightTableRowOutF pubOut3rightTableRowOutFAuto" style="height: calc(100% - 45px);">
    <el-row class="pubOut3rightTableRow" style="height: calc(100%);">
      <el-table height="100%" v-loading="loading" stripe :data="tableData" tooltip-effect="dark" @selection-change="handleSelectionChange">
        <el-table-column v-if="handleSelection=='radio'" fixed="left" label="" width="30">
          <template slot-scope="scope">
            <el-radio :label="scope.$index" v-model="radio" @change="radioChange(scope.row,scope.$index)"></el-radio>
          </template>
        </el-table-column>
        <el-table-column v-else-if="handleSelection!='none'" fixed="left" type="selection" width="30" align='center'></el-table-column>
